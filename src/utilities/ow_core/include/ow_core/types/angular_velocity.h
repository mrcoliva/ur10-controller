...
#ifndef OPEN_WALKER_CORE_ANGULAR_VELOCITY_H
#define OPEN_WALKER_CORE_ANGULAR_VELOCITY_H
...
{
/*!
 * \brief The AngularVelocity class.
 *
 * The AngularVelocity is of type Eigen::Vector3 and is
 * represented by the math symbol \f$\mathbf{\omega}\f$.
 *
 */
template <typename _Scalar>
class AngularVelocity :
    public Eigen::Matrix<_Scalar,3,1>,
    public AngularVelocityBase<AngularVelocity<_Scalar> >,
    TypeGuard
{
  OW_TYPE_GUARD(AngularVelocity)

public:
  typedef _Scalar Scalar;
  typedef Eigen::Matrix<Scalar,3,1> Base;
  typedef AngularVelocityBase<AngularVelocity<Scalar> > ABase;
...
public:
   ...
   */
  template <typename OtherDerived>
  AngularVelocity(const AngularVelocityRef<OtherDerived>& other) :
    Base(other)
  {
  }
  ...
  AngularVelocity& operator=(
    const AngularVelocityRef<OtherDerived>& other)
  {
    Base::operator=(other);
    return *this;
  }
